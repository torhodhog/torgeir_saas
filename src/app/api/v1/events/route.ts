...
export const POST = async (req: NextRequest) => {
  try {
    ...
    if (!user.discordId) {
      return NextResponse.json({ message: "Please insert your discord ID in your account settings" }, { status: 403 })
    }
    ...
    try {
      ...
      console.log("Invalid JSON request body", err)
    ...
    const eventData = {
      ...
      description: validationResult.description || `A new ${category.name} event has been created!`,
