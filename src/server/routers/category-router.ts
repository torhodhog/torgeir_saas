...
import { z } from "zod"
import { CATEGORY_NAME_VALIDATOR } from "../../../src/app/lib/validators/category-validators"
...
export const categoryRouter = router({
  ...
  insertQuickstartCategories: privateProcedure.mutation(async ({ ctx, c }) => {
    const categories = await db.eventCategory.createMany({
      data: [
        { name: "bug", emoji: "🐛", color: parseColor("#ff6b6b") },
        { name: "sale", emoji: "💰", color: parseColor("#ffeb3b") },
        { name: "question", emoji: "🤔", color: parseColor("#6c5ce7") },
      ].map((category) => ({
        ...category,
        userId: ctx.user.id,
      })),
